...
router.get('/',function(req,res){
  ...
  .then(function(vehicles){
    ...
    if(results.length == 0) res.json({result:0, message:'No vehicle found w/ url '});
...
router.get('/validate/:id',function(req,res,next){
  ...
  .then(function(vehicle){
    if(vehicle){
      ...
      res.json({result:1, message:'Vehicle successfully validated w/ url '});
    }
...
router.get('/unvalidate/:id',function(req,res,next){
  ...
  .then(vehicle => {
    ...
    else res.json({result:0, message:'Vehicle not found w/ url '});
  })
...
router.post('/new',function(req,res,next){
  ...
  .then(vehicle => {
    if(vehicle) res.json({result:0, message:'This user has a car yet w/ url 06-004'});
    else{
        ...
        if(user){
          ...
          .then(vehicle => {
            if(vehicle){
              ...
              .catch(err => { res.json({result:-1, message:'Unable to set User to the car w/ url 06-004', error:err}); });
            }
            else res.json({result:0, message:'Vehicle was not created w/ url 06-004'});
          })
          .catch(err => { res.json({result:-1, message:'Unable to create Vehicle w/ url 06-004', error:err}); });
        }
        else res.json({result:0, message:'User not found w/ url 06-004'});
      })
      .catch(err => { res.json({result:-1, message:'Unable to find User w/ url 06-004', error:err}); });
    }
  })
...
router.post('/edit',function(req,res,next){
  ...
  .then(function(vehicle){
    if(vehicle){
      ...
      res.json({result:1, message:'Vehicle successfully updated w/ url 06-005'});
    }
...
router.delete('/:id',function(req,res,next){
  ...
  .then(function(vehicle){
    if(vehicle){
      vehicle.destroy()
      .then(function(vehicle){
        res.json({result:1, message:'Vehicle successfully removed w/ url 06-006'});
      })
